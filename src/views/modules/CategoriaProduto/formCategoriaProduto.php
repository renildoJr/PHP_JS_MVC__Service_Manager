<h1 class="heading hd-1 txt-center"><?= ($model->getId()) ? 'Atualizar Categoria de Produto' : 'Cadastrar Nova Categoria Produto' ?></h1>
<form class="form" action="<?=LINK_CATEGORIA_PRODUTO?>/form/save" method="POST">
    <header class="form__header">
        <button class="btn btn--type-text btn-close fa-solid fa-xmark"></button>
        <h3 class="heading hd-3"><?= ($model->getId()) ? $model->getNome() : 'Nova Categoria de Produto' ?></h3>
    </header>
